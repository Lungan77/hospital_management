...
import { Pill, Stethoscope, Plus, Minus } from "lucide-react";
...
export default function TreatmentForm({ onSubmit, loading }) {
  ...
  const handleTreatmentToggle = (treatmentOption) => {
    setTreatment((prev) => ({
      ...prev,
      treatmentGiven: prev.treatmentGiven.includes(treatmentOption)
        ? prev.treatmentGiven.filter((t) => t !== treatmentOption)
        : [...prev.treatmentGiven, treatmentOption],
    }));
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {commonTreatments.map((treatmentOption) => (
            <label
              key={treatmentOption}
              className="flex items-center gap-3 p-3 border-2 border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-200 cursor-pointer"
            >
              <input
                type="checkbox"
        ...
        <div className="space-y-4">
          {treatment.medications.map((med, index) => {
            ...
            return (
              <div
                key={index}
                className="p-4 border-2 border-purple-200 rounded-xl bg-purple-50"
              >
                  ...
                  <div>
                    ...
                    >
                      ...
                      {selectedMed
                        ...
                        : ["IV", "IM", "Oral", "Sublingual", "Inhalation", "Intranasal"].map(
                            (route) => (
                              <option key={route} value={route}>
                                {route}
                              </option>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          onChange={(e) =>
            setTreatment((prev) => ({ ...prev, patientResponse: e.target.value }))
          }
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          onChange={(e) =>
            setTreatment((prev) => ({ ...prev, additionalNotes: e.target.value }))
          }
          className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white resize-none"
          rows="3"
        />
