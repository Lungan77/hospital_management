...
// Assign Crew Modal Component
function AssignCrewModal({ ambulance, availableCrew, onClose, onAssign }) {
  const [selectedDriver, setSelectedDriver] = useState("");
  const [selectedParamedic, setSelectedParamedic] = useState("");
  const [loading, setLoading] = useState(false);

  // Pre-populate with current crew if any
  useEffect(() => {
    if (ambulance.crew) {
      const driver = ambulance.crew.find(member => member.role === "Driver");
      const paramedic = ambulance.crew.find(member => member.role === "Paramedic");
      
      if (driver) setSelectedDriver(driver.memberId._id || driver.memberId);
      if (paramedic) setSelectedParamedic(paramedic.memberId._id || paramedic.memberId);
    }
  }, [ambulance]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    await onAssign(ambulance._id, selectedDriver, selectedParamedic);
    setLoading(false);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Assign Crew</h2>
              <p className="text-gray-600">{ambulance.callSign} - {ambulance.vehicleNumber}</p>
            </div>
            <button
              onClick={onClose}
              className="p-2 text-gray-400 hover:text-gray-600 transition-colors"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                {ambulance.crew.map((member, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className="text-blue-800">
                      {member.memberId?.name || "Unknown"} - {member.role}
                    </span>
                    <span className="text-blue-600 text-sm">{member.certificationLevel}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            {availableCrew.drivers.length === 0 && (
              <p className="text-sm text-gray-500 mt-2">No available drivers</p>
            )}
          </div>
          ...
          <div>
            ...
            {availableCrew.paramedics.length === 0 && (
              <p className="text-sm text-gray-500 mt-2">No available paramedics</p>
            )}
          </div>

          {/* Assignment Info */}
          <div className="p-4 bg-yellow-50 rounded-xl border border-yellow-200">
            <h4 className="font-semibold text-yellow-900 mb-2">Assignment Information</h4>
            <ul className="text-yellow-800 text-sm space-y-1">
              <li>• Crew members will be assigned to this specific ambulance</li>
              <li>• Previous assignments will be automatically removed</li>
              <li>• Both driver and paramedic are recommended for full operations</li>
              <li>• You can assign just one role if needed</li>
            </ul>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button
              type="submit"
              disabled={loading || (!selectedDriver && !selectedParamedic)}
              className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-blue-800 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
