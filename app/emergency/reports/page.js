...
function EmergencyReports() {
  ...
  useEffect(() => {
    const fetchEmergencies = async () => {
      try {
        ...
        if (res.ok && Array.isArray(data?.emergencies)) {
          setEmergencies(data.emergencies);
        } else {
          console.error('Unexpected response from /api/emergency/list', data);
        }
      } catch (error) {
  ...
  const calculateResponseTime = (e) => {
    if (!e?.dispatchedAt || !e?.onSceneAt) return 'N/A';
    const diff = new Date(e.onSceneAt) - new Date(e.dispatchedAt);
    return `${Math.max(0, Math.floor(diff / 60000))} min`;
  };

  const calculateTransportTime = (e) => {
    if (!e?.transportStartedAt || !e?.arrivedHospitalAt) return 'N/A';
    const diff = new Date(e.arrivedHospitalAt) - new Date(e.transportStartedAt);
    return `${Math.max(0, Math.floor(diff / 60000))} min`;
  };
  ...
  const filteredEmergencies = useMemo(() => {
    ...
    return emergencies.filter((emergency) => {
      const incident = (emergency?.incidentNumber || '').toLowerCase();
      const patient = (emergency?.patientName || '').toLowerCase();
      const address = (emergency?.address || '').toLowerCase();

      const matchesSearch =
        incident.includes(term) || patient.includes(term) || address.includes(term);

      const matchesFilter = filter === 'all' || emergency?.status === filter;

      const reportedAt = emergency?.reportedAt ? new Date(emergency.reportedAt) : null;
      let matchesDate = true;
      if (reportedAt) {
        if (dateRange === 'today') {
          matchesDate = reportedAt.toDateString() === now.toDateString();
        } else if (dateRange === 'week') {
          const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
          matchesDate = reportedAt >= weekAgo;
        } else if (dateRange === 'month') {
          const monthAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
          matchesDate = reportedAt >= monthAgo;
        }
      }

      return matchesSearch && matchesFilter && matchesDate;
    });
  }, [emergencies, searchTerm, filter, dateRange]);
  ...
  const generateEmergencyReportPDF = () => {
    ...
    const responseTimes = filteredEmergencies.filter((e) => e.onSceneAt && e.dispatchedAt);
    const avgResponse =
      responseTimes.length > 0
        ? Math.round(
            responseTimes.reduce((sum, e) => sum + (new Date(e.onSceneAt) - new Date(e.dispatchedAt)), 0) /
              (responseTimes.length * 60000)
          )
        : 'N/A';
    ...
    const stats = [
      ...
      { label: 'Avg Response Time', value: typeof avgResponse === 'number' ? `${avgResponse} min` : 'N/A', color: colors.warning },
    ];
    ...
    const tableData = filteredEmergencies.map((emergency) => [
      ...
      emergency?.reportedAt ? new Date(emergency.reportedAt).toLocaleDateString() : '-',
      emergency?.reportedAt ? new Date(emergency.reportedAt).toLocaleTimeString() : '-',
      emergency?.priority || '-',
      emergency?.status || '-',
    ...
    const completed = filteredEmergencies.filter((e) => e.status === 'Completed');
    const completionRate = Math.round(((completed.length || 0) / (filteredEmergencies.length || 1)) * 100) || 0;
    const criticalCases = filteredEmergencies.filter((e) => e.priority === 'Critical').length;
    const transportTimes = filteredEmergencies.filter((e) => e.transportStartedAt && e.arrivedHospitalAt);

    const avgResponseTime = typeof avgResponse === 'number' ? avgResponse : 0;
    const avgTransportTime =
      transportTimes.length > 0
        ? Math.round(
            transportTimes.reduce((sum, e) => sum + (new Date(e.arrivedHospitalAt) - new Date(e.transportStartedAt)), 0) /
              (transportTimes.length * 60000)
          )
        : 0;

    const performanceData = [
      ['Total Emergency Responses', String(filteredEmergencies.length)],
      ['Completed Successfully', `${completed.length} (${completionRate}%)`],
      ['Critical Priority Cases', String(criticalCases)],
    ...
    autoTable(doc, {
      ...
      theme: 'striped',
      headStyles: { fillColor: colors.success, textColor: [255, 255, 255], fontSize: 10, fontStyle: 'bold' },
      bodyStyles: { fontSize: 9 },
      columnStyles: { 0: { cellWidth: 80, fontStyle: 'bold' }, 1: { cellWidth: 60, halign: 'center' } },
      margin: { left: margin, right: margin },
    });
    ...
    autoTable(doc, {
      ...
      body: priorityBuckets.map(([priority, count]) => [
        priority,
        String(count),
        `${Math.round(((count || 0) / (filteredEmergencies.length || 1)) * 100)}%`,
      ]),
      theme: 'grid',
      headStyles: { fillColor: colors.warning, textColor: [255, 255, 255], fontSize: 10, fontStyle: 'bold' },
      bodyStyles: { fontSize: 9 },
      columnStyles: { 0: { cellWidth: 40 }, 1: { cellWidth: 30, halign: 'center' }, 2: { cellWidth: 30, halign: 'center' } },
      margin: { left: margin, right: margin },
    });
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 mb-8 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={generateEmergencyReportPDF}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-purple-50 rounded-2xl p-4 border border-purple-200">
                <div className="text-2xl font-bold text-purple-600">
                  {(() => {
                    const resTimes = filteredEmergencies.filter((e) => e.onSceneAt && e.dispatchedAt);
                    return resTimes.length > 0
                      ? `${Math.round(
                          resTimes.reduce((sum, e) => sum + (new Date(e.onSceneAt) - new Date(e.dispatchedAt)), 0) /
                            (resTimes.length * 60000)
                        )} min`
                      : '0 min';
                  })()}
                </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredEmergencies.map((emergency) => (
                    <tr key={emergency?._id || emergency?.incidentNumber} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          ...
                          <div className="text-gray-600">{emergency?.patientAge ? `${emergency.patientAge} years` : 'Age unknown'}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 max-w-xs truncate">{emergency?.address}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">{calculateResponseTime(emergency)}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">{calculateTransportTime(emergency)}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-gray-900">
                  {Math.round(
                    ((filteredEmergencies.filter((e) => e.status === 'Completed').length || 0) /
                      (filteredEmergencies.length || 1)) * 100
                  ) || 0}
                  %
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-gray-900">
                  {(() => {
                    const t = filteredEmergencies.filter((e) => e.transportStartedAt && e.arrivedHospitalAt);
                    return t.length > 0
                      ? `${Math.round(
                          t.reduce((sum, e) => sum + (new Date(e.arrivedHospitalAt) - new Date(e.transportStartedAt)), 0) /
                            (t.length * 60000)
                        )} min`
                      : '0 min';
                  })()}
